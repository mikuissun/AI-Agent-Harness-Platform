...
软件工程任务执行与智能体运行平台。目前完成 Stage 1 基础骨架和 Stage 2 Harness Core。任务 API 只保存记录；Harness 核心通过测试适配器验证，不接真实模型。
...
本阶段未实现 LangGraph Workflow、MCP、CLI subprocess、Multi-Agent、Checkpoint、Retry / Recovery、Human Approval、真实 Qwen 调用及前端。工具调用循环仅使用测试 Fake Tool 验证。
