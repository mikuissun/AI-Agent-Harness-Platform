...
软件工程任务执行与智能体运行平台。目前完成 Stage 1–5：基础骨架、Harness Core、受控 CLI / MCP、持久化 Workflow，以及角色 Agent、有限重试与人工审批。任务 API 只保存记录，不接真实模型。最新设计与边界见 [Stage 5 文档](docs/stage5.md)。
...
- Stage 5：Multi-Agent + Retry + Approval（已完成）
- Stage 6：E2E + UI + README（未实现，当前仅有基础 README）

本阶段未实现真实 Qwen 调用、前端和复杂 Recovery。角色协作、有限重试与人工审批通过 Stage 5 的 Scripted Model 和集成测试验证。
